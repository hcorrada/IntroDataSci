...
We are now entering the last step of what we would want to do with a dataset before starting modeling using statistics or Machine Learning. We have seen manipulations and operations that prepare datasets into tidy (or normal form), compute summaries, remove or impute missing data and join tables to obtain organized, clean data tables that contain the observational units, or entities, we want to model. 

At this point, we want to perform additional _Exploratory Data Analysis_ to better understand the data at hand, and help us make decisions about appropriate modeling methods, or data transformations that may be helpful to do. Moreover, there are many instances where data modeling is not required to tell a clear and convincing story with data. Many times an effective visualization can lead to convincing conclusions.
