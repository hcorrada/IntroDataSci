...
In comparison with the linear model only using `weight`, this multivariate model explains _more of the variance_ of `mpg`, but using more and more predictors. This is where the notion of _degrees of freedom_ comes in: we now have a model with expanded _representational_ ability. 
...
Columns `AIC` and `BIC` include this type of statistic that penalizes model fit with model size. The smaller this value, the better. Let's now compare a model only using `weight`, a model only using `weight` and `year` and the full multiple regression model we saw before.
...
Now let's build a to capture the relationship between `salary` and `major`: 
...
Let's fit the model and see if that observation how much statistical confidence we can give to those observations:
