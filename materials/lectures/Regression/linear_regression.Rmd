...
Linear regression is a very elegant, simple, powerful and commonly used technique for data analysis. We use it extensively in exploratory data analysis (we used in project 1, for example) and in statistical analyses since it fits into the framework we saw in the last unit, and thus let's do things like construct confidence intervals and hypothesis testing for relationships between variables.

## Simple Regression

Let's start with the simplest linear model. The goal here is to analyze the relationship between a _continuous numerical_ variable $Y$ and another (_numerical_ or _categorical_) variable $X$. We assume that in our population of interest the relationship between the two are given by a linear function:
