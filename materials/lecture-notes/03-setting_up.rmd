...
Along with Python it is one of the most popular environments for data analysis (e.g., figure below from [KDNuggets 2018 software survey](https://www.kdnuggets.com/2018/05/poll-tools-analytics-data-science-machine-learning-results.html)

![](img/kdnuggets-2018.jpg)
...
Resources for learning and reading about R are listed in our [here](http://www.hcbravo.org/IntroDataSci/resources/). Of note are the [swirl project](http://swirlstats.com/) and DataCamp's [introduction to R] course.
...
We provide a simple script to install most of the R packages used in this class. Download the script here: 

[http://www.hcbravo.org/IntroDataSci/misc/cmsc320_install_libs.R](http://www.hcbravo.org/IntroDataSci/misc/cmsc320_install_libs.R)

On the command line, or on the Rstudio terminal tab, run the following:

```
Rscript cmsc320_install_libs.R
```

## Python/Jupyter

To use python, we recommend to use [Anaconda](https://www.anaconda.com/) a great platform for package and software management, especially for data science.

For the purposes of the class, it is sufficient to download the streamlined `Miniconda` distribution:

https://docs.conda.io/en/latest/miniconda.html

We assume you installed the Python3 version.

To install the tools needed for the course, we have provided an environment file that lists all software we will use. Download the file here:

[http://www.hcbravo.org/IntroDataSci/misc/cmsc320_conda.yml](http://www.hcbravo.org/IntroDataSci/misc/cmsc320_conda.yml)

To create an
Anaconda environment use `conda env create -f=cmsc320_conda.yml -n cmsc320`

Once installed and loaded (with `conda activate cmsc320`, we recommend you use jupyter lab for this course. Start it using `jupyter lab`.

There you can create jupyter notebooks to carry out your work.

Final note: most of the support for the libraries included in the `tidyverse` is
given by the `pandas` python package:

https://pandas.pydata.org/

We are using the tidyverse's `ggplot2` package for plotting. There is a `plotnine` python package available that supports most of the same functionality. It is included in
the environment we have provided.
